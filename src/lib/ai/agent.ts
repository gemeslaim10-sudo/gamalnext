...
export class AiAgent {
    ...
    async run(message: string, chatHistory: any[], userContext?: any, sessionId?: string) {
        try {
            // 1. RAG Discovery
    ...
    private async executeTools(calls: any[]) {
        const responses: any[] = [];
    ...
    private async handleLeadCapture(text: string, ctx?: any) {
     ...
     */
    private async runFailover(message: string, history: any[]) {
        const providers = [
